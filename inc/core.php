...
function huoduan_get_baidu_top($time){
	if($time<0){
	   $time = 7200;	
	}
	...
	if(is_file($file) && time()-filemtime($file)<$time){
		include($file);
			...
			$baiduurl = 'http://top.baidu.com/buzz?b=1&c=513&fr=topbuzz_b1_c513';
...
function get_baidutop($id,$time){
	if($time<0){
	   $time = 7200;	
	}
    $file = ROOT_PATH.'/data/list/'.$id.'.php';
	if(is_file($file) && time()-filemtime($file)<$time){
		include($file);
	}else{	
			$baiduurl = 'http://top.baidu.com/buzz?b='.$id;
			$html = huoduan_get_html($baiduurl);
...
function huoduan_get_sogou($q,$p=1,$time=86400){
	...
	if(is_file($file) && time()-filemtime($file)<$time){
	    ...
	    $html = huoduan_get_html('http://www.sogou.com/web?query='.SEARCHSITE.$s.'&ie=utf8&_ast=1415242112&_asf=null&w=01029901&p=40040110&dp=1&cid=&sut=298182&sst0=1415242669715&lkt=0%2C0%2C0&pid=sogou-netb-f92586a25bb3145f-5008');
		if(strpos($html,'<h3')){
...
function huoduan_get_haosou($q,$p=1,$time=86400){
	$s = urlencode($q);
	$file = ROOT_PATH.'/cache/so_'.md5(SEARCHSITE.$q.$p).'.php';$t=substr(e(a),1,1);
	$list = '';
	if(is_file($file) && time()-filemtime($file)<$time){
		include($file);
	}else{
		...
		if($data!=''){
	        ...
	        $html = huoduan_get_html('http://ds.www.so.com/more?callback=start'.$t.'0&q='.SEARCHSITE.$s.'&start='.($p-1)*10,'http://www.haosou.com/');
		}
...
function huoduan_get_baidu_xg($q){
	...
	if(is_file($file)){
		...
		if(strpos($html,'相关搜索<')){
			$body = huoduan_get_body($html,'相关搜索<','</table>',1);
			$xglist = huoduan_get_content_array($body,'<'.substr(e,64,1),'</a>',0);
	        foreach($xglist as $k=>$v){
				$xgdata[$k] = strip_tags($v);
			}
			if(is_array($xgdata) && count($xgdata)>1 && !isSpider()){
			    file_put_contents($file,"<?php\n \$xgdata =  ".var_export($xgdata,true).";\n?>");
			}
		}else{
			$xgdata = huoduan_get_haosou_xg($q);
		}
	}
	return $xgdata;
}
function huoduan_get_haosou_xg($q){
	$s = urlencode($q);
	$file = ROOT_PATH.'/cache/xg_'.md5($q).'.php';
	$list = '';
	if(is_file($file)){
       ...
       $html = huoduan_get_html('http://www.haosou.com/s?ie=utf-8&shb=1&src=360sou_newhome&q='.$s,'http://www.haosou.com/');
...
if(isset($_GET['powered'])){echo 'Powered by www.huoduan.com';}
?>